...
function closeMobileOverlay(kind){const el=document.querySelector(kind);if(!el)return false;el.classList.add('hidden');if(location.hash.startsWith('#/detail/'))history.back();return true}
function bindTouch(){document.addEventListener('click',e=>{const b=e.target.closest('[data-action="close-detail"],[data-action="close-search"]');if(!b)return;e.preventDefault();e.stopPropagation();if(b.dataset.action==='close-detail')closeMobileOverlay('#detail');else closeMobileOverlay('#search')},{capture:true,passive:false})}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{build();bindTouch()});else{build();bindTouch()}
})();
