...
function bindTouch(){document.addEventListener('click',e=>{const b=e.target.closest('[data-action="close-detail"],[data-action="close-search"]');if(!b)return;e.preventDefault();if(b.dataset.action==='close-detail')window.goBack?.();else if(b.dataset.action==='close-search')window.closeSearch?.()},{passive:false});}
