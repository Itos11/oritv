...
function touchStyles(){if(window.innerWidth>700)return;document.querySelectorAll('.cards').forEach(x=>x.style.touchAction='pan-x pan-y');document.querySelectorAll('.cards .card').forEach(x=>x.style.touchAction='auto');document.querySelectorAll('.grid .card,.back-btn,.action,.mobile-nav button').forEach(x=>x.style.touchAction='manipulation')}
